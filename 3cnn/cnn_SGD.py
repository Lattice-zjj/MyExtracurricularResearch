...
class cnn_mnist(nn.Module):                  #CNN继承了nn.Module 类
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(in_channels=out_chan2, out_channels=out_chan3, kernel_size=kernel_size_cov, stride=1, padding=1)   

        self.fc1 = nn.Linear(in_features=49*out_chan3, out_features=features)
...
#showimg(make_grid(batch[0],nrow=10,padding=2,pad_value=1),'训练集部分数据')   

#初始化
cnn=cnn_mnist()
criterion = nn.CrossEntropyLoss()    #交叉熵损失函数
optimizer = torch.optim.SGD(cnn.parameters(), lr=learning_rate,weight_decay=0.001)  #优化器
